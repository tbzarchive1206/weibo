...
function MediaTile({ media, showName = false }: { media: Media; showName?: boolean }) {
  if (media.kind === "video") return <figure className="media-tile video-tile"><iframe src={previewUrl(media.id)} title="Weibo video" allow="autoplay; fullscreen" loading="lazy" /><MediaActions media={media} showName={showName} /></figure>;
  if (media.kind === "audio") return <figure className="media-tile audio-tile"><div className="audio-mark" aria-hidden="true">AUDIO / WEIBO</div><iframe src={previewUrl(media.id)} title="Weibo audio" allow="autoplay" loading="lazy" /><MediaActions media={media} showName={showName} /></figure>;
...
export function WeiboArchive({ data }: { data: Archive }) {
  const now = new Date();
  ...
  return <main id="top">
    ...
    {!collection ? <section className="member-picker insta-picker weibo-picker">
      ...
      <button className="other-profile-tile" onClick={() => chooseCollection(data.special)}><span>{String(data.profiles.length + 1).padStart(2, "0")} / SPECIAL COLLECTION</span><strong>WEIBO UPDATES RELATED TO THE BOYZ</strong><em>THE BOYZ 相关微博更新</em><small>{data.special.media.length.toLocaleString("en-US")} MEDIA FILES →</small></button>
    </section> : <section className="member-gallery insta-gallery">
      <header className="member-gallery-head"><button onClick={() => setCollection(null)}>← ALL PROFILES</button><div><span>WEIBO MEDIA / PROFILE</span><h2>{collection.name.toUpperCase()}</h2></div><a href={folderUrl(collection.id)} target="_blank" rel="noreferrer">OPEN FOLDER ↗</a></header>
