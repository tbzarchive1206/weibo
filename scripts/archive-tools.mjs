...
export const ROOT_FOLDER_ID = "1aLz7FnwaQcrFv926AOrBxic67qvAcUdk";
export const ROOT_TITLE = "WEIBO MEDIA ARCHIVE";
...
export function normalizeArchive(raw) {
  ...
  const specialFolder = topFolders.find((folder) => /WEIBO UPDATES RELATED TO THE BOYZ/iu.test(folder.name));
  const profiles = topFolders.filter((folder) => folder.id !== specialFolder?.id).map((folder) => collectionFor(raw, folder, false));
  const special = specialFolder ? collectionFor(raw, specialFolder, true) : { id: "", name: "Weibo Updates Related to THE BOYZ", media: [] };
